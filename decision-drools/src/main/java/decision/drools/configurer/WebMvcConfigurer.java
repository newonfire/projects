...
import decision.drools.core.ServiceException;
		...
		exceptionResolvers.add(new HandlerExceptionResolver() {
			public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response,
				...
				if (e instanceof ServiceException) {// 业务失败的异常，如“账号或密码错误”
					result.setCode(ResultCode.FAIL).setCodeDesc(e.getMessage());
					logger.error(e.getMessage(), e);
				} else if (e instanceof NoHandlerFoundException) {
					result.setCode(ResultCode.NOT_FOUND).setCodeDesc("接口 [" + request.getRequestURI() + "] 不存在");
					logger.error(e.getMessage(), e);
					...
					result.setCode(ResultCode.INTERNAL_SERVER_ERROR)
							.setCodeDesc("接口 [" + request.getRequestURI() + "] 内部错误，请联系管理员");
					String message;
			...
			registry.addInterceptor(new HandlerInterceptorAdapter() {
				...
				public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
					...
					if (pass) {
						...
						Result result = new Result();
						result.setCode(ResultCode.UNAUTHORIZED).setCodeDesc("签名认证失败");
						responseResult(response, result);
						return false;
					}
				}
